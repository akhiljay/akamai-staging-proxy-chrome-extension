...
ProxyFormController.prototype = {
  ...
  get bypassList() {
    ...
    /**
   * @return {Array<string>} A list of hostnames that should be proxied over to staging
   */
  get hostList() {
    return document.getElementById('hostList').value.split(/\s*(?:,|^)\s*/m);
  },
  ...
  set bypassList(data) {
   ...
   /**
   * @param {?Array<string>} data A list of hostnames that should be proxied over to staging.
   *  If empty, the host list is emptied.
   */
  set hostList(data) {
    if (!data)
      data = [];
    document.getElementById('hostList').value = data.join(', ');
  },
...
///////////////////////////////////////////////////////////////////////////////
  ...
  applyChanges_: function(e) {
    //trigger API request to server here 
    //console.log(this.hostList);
    var arr_hostList = this.hostList;
    var arr_hostListlength = arr_hostList.length;
    console.log(arr_hostListlength);

    var arr_apihost = [];
    var active = document.getElementsByClassName('active')[0];
    switch (active.id) {
      case ProxyFormController.ProxyTypes.SYSTEM:
      chrome.runtime.sendMessage({type: "gaq", target: "System proxy settings", behavior: "enabled"});
    ...
    if (arr_hostList && arr_hostList.length){
      var JSONpayload = '{"hosts":['+ JSONarr + ']}';
      //console.log(JSONpayload);
      //send request to proxy server
      //ConfigureProxyServer_(JSONpayload);
      chrome.runtime.sendMessage({
        msg: "sending_post_body", 
        data: {
            subject: "MSG",
            content: JSONpayload
        }
    });
    chrome.runtime.sendMessage({type: "gaq", target: "staging hosts", behavior: "entered"});

    }
    else{
      chrome.runtime.sendMessage({type: "gaq", target: "staging hosts", behavior: "not entered"});
   ...
   * @private
   */
  ConfigureProxyServer_: function(jsonpost) {
    console.log(jsonpost);
    sendPostReq(jsonpost);
  },
  ...
  toggleIncognitoMode_: function(e) {
    ...
    if (!this.isAllowedIncognitoAccess_) {
      var msg = "Sorry, I'm afraid I can't do that. Give me access " +
